    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="downGoods(props.item)">下架</v-btn>
          <v-btn icon v-else @click="upGoods(props.item)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
  ...
  export default {
    ...
    methods: {
      ...
      async editGoods(oldGoods) {
        // 发起请求，查询商品详情和skus
        oldGoods.spuDetail = await this.$http.loadData("/item/spu/" + oldGoods.id);
      ...
      async downGoods(oldGoods){
        this.$http.put('/item/spu/out/'+oldGoods.id)
        .then(({data})=>{
      ...
      async upGoods(oldGoods){
        this.$http.put('/item/spu/up/'+oldGoods.id)
        .then(({data})=>{
